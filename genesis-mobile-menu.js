jQuery(document).ready(function($) {
	function genesis_mobile_menu() {
		...
		if( $(window).width() < genesis_mobile_breakpoint && $('.nav-primary').is(":visible") ) {
		...
		else if( $(window).width() > genesis_mobile_breakpoint && $('.nav-primary').is(":hidden") ) {
			$('.toggle-menu').remove();
	...
	genesis_mobile_menu(genesis_mobile_breakpoint);
	$(window).resize(function() { genesis_mobile_menu(genesis_mobile_breakpoint); });
	var width = $(window).width() - 32;
	$( '.genesis-mobile-menu' ).css( 'marginLeft', '-' + width + 'px' );
	$( '.genesis-mobile-menu' ).css( 'width', width + 'px' );
	$( 'html' ).css( 'backgroundPosition', (width - 9) + 'px 0' );
